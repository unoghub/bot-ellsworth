...
export async function setup_gamejam(interaction?: RepliableInteraction) {
  ...
  const jammer_role = await createJammerRole(interaction, { new: new_guild });
  ...
  const channel = await create_panel_channel({ new: new_guild });
  ...
  const teams_forum = await create_teams_channel({ new: new_guild });
  ...
  const archive_category = await create_archive_category({ new: new_guild });
